...
export default defineEventHandler(async (event) => {
  try {
    ...
    const has_hero_image =
      query.has_hero_image !== undefined
        ? query.has_hero_image === "true"
        : undefined;
    ...
    logger.info("🔍 Subjects search params:", {
      name_pattern,
      tags,
      tag_match_mode,
      has_hero_image,
    ...
    if (tags && tags.length > 0) {
      const tagConditions = tags.map((tag) => {
        if (tag_match_mode === "partial") {
          ...
          return sql`EXISTS (SELECT 1 FROM jsonb_array_elements_text(${
            subjects.tags
          }->'tags') AS tag_elem WHERE tag_elem ILIKE ${`%${tag}%`})`;
        } else {
      ...
      queryWithSort =
        sortDirection === "desc"
          ? queryWithWhere.orderBy(
              desc(
                sql`(SELECT COUNT(*) FROM ${jobs} WHERE ${jobs.subjectUuid} = ${subjects.id})`
              )
            )
          : queryWithWhere.orderBy(
              asc(
    ...
    logger.info(
      `✅ Found ${processedSubjects.length} subjects (${totalCount} total)`
    );
    logger.info(`⏱️ Processing results: ${processTime}ms`);
    logger.info(
      `⏱️ Total time: ${totalTime}ms (query: ${queryTime}ms, count: ${countTime}ms, process: ${processTime}ms)`
    );
