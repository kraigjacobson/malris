// Compare anus/pussy DETAIL helpers on the facesitting poses: the new Genitals
// helper (trigger "Innie vagina") alone vs. Genitals + PussyLoRA together.
// char + facesitting@0.8. Low-res, pushed to front.
...
const GENITALS = 't2v/genitals_helper_v1.0_e219.safetensors' // single noise-agnostic file -> fills both halves
...
// "Innie vagina" is the Genitals-helper trigger (small tucked labia).
const tail = ' Innie vagina. photorealistic, extreme detail, sharp focus, macro close-up detail on her anus and pussy.'
...
const POSES = {
  ...
  'fwd-descend': `POV first-person view looking straight up as ${BODY} kneeling and straddling the viewer's head facing the viewer, slowly lowering her hips down toward the camera — her whole naked body and face stay visible above as she descends, until she sits fully down and her wet pussy and anus fill the frame. She gazes down at the viewer, her face fully visible.`,
  'turn-around-reverse': `POV first-person view looking straight up as ${BODY} turning around and straddling the viewer's head facing away, her ass and anus toward the camera, then slowly lowering down until her anus fills the frame, her wet pussy just below. She looks back over her shoulder down at the viewer, her face visible.`,
}
// helper slots (slot 3 = detail helper A, slot 4 = detail helper B). Genitals is a
// single file, so high==low.
const CONFIGS = [
  { label: 'genitals0.7', s3: { hi: GENITALS, lo: GENITALS, str: 0.7 } },
  { label: 'genitals0.6+pussy0.6', s3: { hi: GENITALS, lo: GENITALS, str: 0.6 }, s4: { hi: PUSSY_HI, lo: PUSSY_LO, str: 0.6 } },
]
...
for (const [pose, scene] of Object.entries(POSES)) {
  for (const c of CONFIGS) {
    const params = {
      ...
      lora_3_high: c.s3.hi, lora_3_low: c.s3.lo, lora_3_high_strength: c.s3.str, lora_3_low_strength: c.s3.str,
    }
    if (c.s4) Object.assign(params, {
      lora_4_high: c.s4.hi, lora_4_low: c.s4.lo, lora_4_high_strength: c.s4.str, lora_4_low_strength: c.s4.str,
    })
...
console.log(`\n${created.length} helper-comparison jobs queued + prioritized.`)
await pool.end()
