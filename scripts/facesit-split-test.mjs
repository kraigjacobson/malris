// Split-noise test: hold the facesitting LoRA's HIGH-noise strong (pose) but drop
// its LOW-noise (its soft/overcooked detail), and let the detail HELPER's LOW-noise
// carry the sharpness. char + facesitting + PussyLoRA/Genitals. Low-res, front.
...
const GENITALS = 't2v/genitals_helper_v1.0_e219.safetensors'
...
// fs = facesitting {high,low}; helper slot 3 with its own {high,low}.
const CONFIGS = [
  { label: 'splitA-fsH.9L.5-pussyH.4L.8', fs: { h: 0.9, l: 0.5 }, s3: { hi: PUSSY_HI, lo: PUSSY_LO, h: 0.4, l: 0.8 } },
  { label: 'splitB-fsH.9L.4-pussyH.3L.9', fs: { h: 0.9, l: 0.4 }, s3: { hi: PUSSY_HI, lo: PUSSY_LO, h: 0.3, l: 0.9 } },
  { label: 'splitGen-fsH.9L.5-genH.4L.8', fs: { h: 0.9, l: 0.5 }, s3: { hi: GENITALS, lo: GENITALS, h: 0.4, l: 0.8 } },
]
...
for (const [pose, scene] of Object.entries(POSES)) {
  for (const c of CONFIGS) {
    const params = {
      ...
      lora_2_high: FS_HI, lora_2_low: FS_LO, lora_2_high_strength: c.fs.h, lora_2_low_strength: c.fs.l,
      lora_3_high: c.s3.hi, lora_3_low: c.s3.lo, lora_3_high_strength: c.s3.h, lora_3_low_strength: c.s3.l,
    }
...
console.log(`\n${created.length} split-noise jobs queued + prioritized.`)
await pool.end()
