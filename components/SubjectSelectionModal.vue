  ...
  <UModal v-model:open="isOpen" :ui="{ width: 'max-w-4xl' }">
    <template #header>
      <div class="flex items-center justify-between w-full">
        ...
        <UButton color="neutral" variant="ghost" icon="i-heroicons-x-mark-20-solid" class="-my-1 ml-auto" @click="isOpen = false" />
      </div>
    </template>
    ...
    <template #body>
      ...
      <div class="p-3 sm:p-4 space-y-3" :class="{ 'border-b border-gray-200 dark:border-gray-700': hasSearched || loading || error }">
        <!-- Tags Search -->
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2"> Search by Tags </label>
          <div class="space-y-2">
            <UInputTags v-model="searchStore.subjectSearch.selectedTags" placeholder="Add tags (e.g., portrait, landscape)" class="w-full" enterkeyhint="enter" :ui="{ trailing: 'pe-1' }">
              <template v-if="searchStore.subjectSearch.selectedTags?.length" #trailing>
                <UButton color="neutral" variant="link" size="sm" icon="i-lucide-circle-x" aria-label="Clear all tags" @click="searchStore.subjectSearch.selectedTags = []" />
              </template>
            </UInputTags>
          </div>
        </div>
      ...
      <div v-if="selectedVideo && displayImages" class="p-6 border-b border-gray-200 dark:border-gray-700">
        <div class="flex justify-center">
          <div class="relative w-80 h-60 bg-gray-200 dark:bg-gray-700 overflow-hidden group cursor-pointer" :data-video-uuid="selectedVideo.uuid" @mouseenter="handleVideoHover(selectedVideo.uuid, true)" @mouseleave="handleVideoHover(selectedVideo.uuid, false)">
            <!-- Video element with poster -->
            <video ref="videoElement" :poster="selectedVideo.thumbnail_uuid ? `/api/media/${selectedVideo.thumbnail_uuid}/image?size=sm` : undefined" class="w-full h-full object-cover object-top" muted loop preload="none" playsinline webkit-playsinline>Your browser does not support the video tag.</video>
          </div>
        </div>
      </div>
      ...
      <div class="p-3 sm:p-4 max-h-[60vh] overflow-y-auto">
        <SubjectGrid :subjects="subjects" :loading="loading" :loading-more="loadingMore" :has-searched="hasSearched" :has-more="hasMore" :error="error" :selection-mode="true" :display-images="displayImages" :disable-infinite-scroll="true" @subject-click="selectSubject" @load-more="loadMore" />
      </div>
    </template>

    <template #footer>
      <div class="flex justify-center gap-3">
        <UButton color="primary" size="sm" icon="i-heroicons-magnifying-glass" @click="performSearch"> Search Subjects </UButton>
        <UButton v-if="hasSearched || searchQuery || searchStore.subjectSearch.selectedTags.length > 0" color="gray" variant="outline" size="sm" icon="i-heroicons-x-mark" @click="clearAll"> Clear </UButton>
      </div>
    </template>
  </UModal>
</template>
...
// Use the search store, subjects composable, and settings
